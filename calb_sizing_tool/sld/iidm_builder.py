...
def build_network_for_single_unit(snapshot: dict) -> "pp.network.Network":
    if pp is None:
        raise ImportError("pypowsybl is required to build the IIDM network.")
    ...
    vl_mv_id = "VL_MV_01"
    vl_lv_id = "VL_LV_01"
    bus_mv_main_id = "BBS_MV_MAIN"
    bus_mv_rmu_id = "BBS_MV_RMU"
    bus_lv_main_id = "BBS_LV_MAIN"

    grid_mv_kv = _safe_float(mv.get("kv"), 33.0)
    pcs_lv_v = _safe_float(transformer.get("lv_v"), 690.0)
    pcs_lv_kv = pcs_lv_v / 1000.0 if pcs_lv_v > 20.0 else _safe_float(
        transformer.get("lv_kv"), 0.69
    )

    transformer_rating_mva = _safe_float(transformer.get("rated_mva"), 0.0)
    if transformer_rating_mva <= 0.0:
        transformer_rating_mva = _safe_float(transformer.get("rated_kva"), 0.0) / 1000.0
    ...
    net.create_switches(
        id="RMU_01",
        voltage_level_id=vl_mv_id,
        bus1_id=bus_mv_main_id,
        bus2_id=bus_mv_rmu_id,
        name="RMU",
        kind="DISCONNECTOR",
        open=False,
    )
    ...
    net.create_2_windings_transformers(
        id="TR_01",
        voltage_level1_id=vl_mv_id,
        bus1_id=bus_mv_rmu_id,
        voltage_level2_id=vl_lv_id,
        bus2_id=bus_lv_main_id,
        rated_u1=grid_mv_kv,
        rated_u2=pcs_lv_kv,
        rated_s=transformer_rating_mva,
        r=0.01,
    ...
    # Keep all PCS injections on the LV main bus to avoid extra LV busbars in the raw SLD.
    for idx, feeder in enumerate(feeders, start=1):
        pcs_id = feeder.get("pcs_id") or f"PCS-{idx:02d}"
        pcs_rating_kva = _safe_float(feeder.get("pcs_rating_kva"), 0.0)
        pcs_rating_mw = pcs_rating_kva / 1000.0 if pcs_rating_kva else 0.1

        net.create_generators(
            id=pcs_id,
            voltage_level_id=vl_lv_id,
            bus_id=bus_lv_main_id,
            min_p=0.0,
            max_p=pcs_rating_mw,
            target_p=0.0,
            target_q=0.0,
